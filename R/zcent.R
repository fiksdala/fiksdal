#' zcent
#'
#' This function produces centered and/or standardized variables. Output is
#' either full dataset with standardized/centered vars or just the selected vars
#' in a dataframe.
#' @param df dataframe
...
zcent <- function(df,vars,outtype='both',merged=TRUE) {
  tdf <- df[vars]
  ...
  if(outtype=='z'|outtype=='both') {
    for(i in 1:length(tdf)){
      stdlist[[i]] <- (tdf[i]-mean(tdf[,i], na.rm=TRUE))/sd(tdf[,i],na.rm=TRUE)
      names(stdlist[[i]]) <- paste('z', names(stdlist[[i]]), sep='')
    }
  }
  if(merged==TRUE) {
    if(outtype=='both') {
